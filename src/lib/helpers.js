...
export function formatMoney(
  num,
  maxSignificantFigures = 6,
  maxDecimalPlaces = 3,
) {
  let numberShort = formatNumberShort(
    num,
...
export function formatNumberShort(
  ...
  if (Math.abs(num) > 10 ** (3 * symbols.length))
    ...
    return num.toExponential(
      Math.min(maxDecimalPlaces, maxSignificantFigures - 1),
    );
  for (
  ...
  return (
    ...
    num.toFixed(
      Math.max(
        ...
        Math.min(
          maxDecimalPlaces,
          maxSignificantFigures - Math.floor(1 + Math.log10(num)),
        ),
      ),
    ) +
    symbols[i]
...
export function parseShortNumber(text = "0") {
  ...
  for (const sym of symbols.slice(1))
    if (text.toLowerCase().endsWith(sym))
      return (
        Number.parseFloat(text.slice(0, text.length - sym.length)) *
        Math.pow(10, 3 * symbols.indexOf(sym))
      );
  return Number.NaN;
}
...
export function formatNumber(
  ...
  return num == 0.0
    ...
    : num.toFixed(
        Math.max(
          minDecimalPlaces,
          Math.max(0, minSignificantFigures - Math.ceil(Math.log10(num))),
        ),
      );
}
...
export function formatDuration(duration) {
  ...
  seconds =
    hours == 0 && minutes == 0 ? seconds.toPrecision(3) : seconds.toFixed(0);
  if (seconds > 0) {
...
export function disableLogs(ns, listOfLogs) {
  ["disableLog"]
    .concat(...listOfLogs)
    .forEach((log) => checkNsInstance(ns, '"disableLogs"').disableLog(log));
}
...
export function getFilePath(file) {
  const subfolder = "alainbryden/"; // git-pull.js optionally modifies this when downloading
...
export async function runCommand(
  ...
  return await runCommand_Custom(
    ns,
    ns.run,
    command,
    fileName,
...
export async function runCommand_Custom(
  ...
  if (!Array.isArray(args))
    throw new Error(
      `args specified were a ${typeof args}, but an array is required.`,
    );
  ...
  let script =
    (required.length > 0
      ? `import { ${required.join(", ")} } from 'alainbryden/helpers.js'\n`
...
export async function waitForProcessToComplete_Custom(
  ...
  for (var retries = 0; retries < 1000; retries++) {
    ...
    if (verbose && retries % 100 === 0)
      ns.print(
        `Waiting for pid ${pid} to complete... (${formatDuration(Date.now() - start)})`,
      );
...
export async function autoRetry(
  ...
  while (attempts++ <= maxRetries) {
    try {
      ...
      const error =
        typeof errorContext === "string" ? errorContext : errorContext();
      if (!fnSuccessCondition(result)) throw asError(error);
      return result;
    } catch (error) {
      ...
      log(
        ...
        `${fatal ? "FAIL" : "INFO"}: Attempt ${attempts} of ${maxRetries} failed` +
          (fatal
            ...
            : `. Trying again in ${retryDelayMs}ms...`),
...
export async function getActiveSourceFiles(
  ...
  return await getActiveSourceFiles_Custom(
    ns,
    getNsDataThroughFile,
    includeLevelsFromCurrentBitnode,
  );
}
...
export async function getActiveSourceFiles_Custom(
  ...
  if (includeLevelsFromCurrentBitnode) {
    try {
      const currentNode = (
        await fnGetNsDataThroughFile(
          ns,
          "ns.getResetInfo()",
          "/Temp/reset-info.txt",
        )
      ).currentNode;
      dictSourceFiles[currentNode] = Math.max(
        3,
        dictSourceFiles[currentNode] || 0,
      );
    } catch {
...
export async function tryGetBitNodeMultipliers_Custom(
  ...
  try {
    canGetBitNodeMultipliers =
      5 in (await getActiveSourceFiles_Custom(ns, fnGetNsDataThroughFile));
  } catch {}
...
export function getConfiguration(ns, argsSchema) {
    ...
    try {
      ...
      for (const key in parsedOverrides) {
        ...
        if (!match)
          ...
          match &&
          JSON.stringify(match[1]) != JSON.stringify(override)
        ) {
  ...
  try {
    ...
    log(
      ns,
      (error
        ? `ERROR: There was an error parsing the script arguments provided: ${error}\n`
        : "INFO: ") +
        ...
        argsSchema
          .map(
            (a) =>
              ...
              (a[0] in optionDescriptions
                ? ` // ${optionDescriptions[a[0]]}`
                : ""),
          )
          .join("") +
        "\n" +
...
export function unEscapeArrayArgs(args) {
  ...
  const escapeChars = ['"', "'", "`"];
  return args.map((arg) =>
    escapeChars.some((c) => arg.startsWith(c) && arg.endsWith(c))
      ? arg.slice(1, -1)
      : arg,
  );
}
