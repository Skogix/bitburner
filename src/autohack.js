...
//
import { deepScan, updatePC } from "./deepScan.js";
...
async function runProtocol(ns, PC) {
  // Activate the Run Protocol Function
  ns.tprint("huhuhu");
  ns.tprint(PC);
  const file = `${PC.protocol}Protocol.js`; // Converting Protocol parameter to filename
  if (PC.name == undefined) {
    ns.tprint("undefined");
  } else {
    ns.scp(`${file}`, PC.name); // Perform the precise file upload
    ns.tprint(
      `Targeting ${PC.target} with ${file} on` + // Notify terminal of remote execution
        ` ${PC.name} using ${PC.threads} threads.`,
    ); // Notification continued... announce thread count
...
function getTarget(ns, PCAR) {
  ...
  if (IO.length > 0) {
    // If the list has any available PCs...
    return {
      // Return a target with the following:
...
export async function main(ns) {
  ...
  let PCAR = deepScan(ns); // Build network database from scratch
  while (true) {
    // Set a forever loop (main loop)
    ...
    for (let pc of PCAR) {
      // Cycle through the entire PC Array
      if (pc.idle && pc.protocol) {
        // Find PCs that have pending jobs
        if (pc.type === "node" && pc.protocol == "pending") {
          // Find PCs that are support nodes
