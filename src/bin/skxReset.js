...
export async function main(ns) {
  ...
  if (startsWith && endsWith) {
    filesToRemove.push(
      ...files.filter((f) => f.startsWith(endsWith) && f.endsWith(endsWith)),
    );
  }
  ...
  filesToRemove = filesToRemove.filter((f) =>
    ["js", "ns", "txt", "script"].some((ext) => f.endsWith(ext)),
  );
...
`);
  ...
  if (confirmedDelete) {
    ...
    ns.tprint(
      `Aborted deletion of ${filesToRemove.length} files from '${options.host}'`,
    );
  }
}
