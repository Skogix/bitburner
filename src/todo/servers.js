...
export const getServersWithThreadCapacity = (ns, threadsRequired, script) => {
	...
	const targets = getRootedServers(ns)
		...
		.filter(
			(server) => ns.getServerMaxRam(server) - ns.getServerUsedRam(server) > 0,
		);
	let target;
	let i = 0;

	// Create a list of all servers which can handle this task by itself
	const capableservers = targets.filter(
		(server) =>
			Math.floor(
				(ns.getServerMaxRam(server) - ns.getServerUsedRam(server)) / scriptSize,
			) >= threadsRequired,
	);
	...
	if (capableservers.length) {
		...
		while (threadsRequired > 0 && (target = targets[i++])) {
			const threadsAvailable = Math.floor(
				(ns.getServerMaxRam(target) - ns.getServerUsedRam(target)) / scriptSize,
			);
...
export const serversForEach = (ns, fn) => {
	...
	const serverIterator = ([hostname, server], ancestors = []) => {
		...
		if (typeof server.connections == "object") {
			Object.entries(server.connections).forEach((entry) =>
				serverIterator(entry, [...ancestors, hostname]),
			);
		}
	};
...
export const getHackableServers = (ns, includePurchased = false) => {
	...
	const servers = getRootedServers(ns)
		.filter((server) => server !== "home")
		.filter(
			(server) => ns.getServerRequiredHackingLevel(server) <= hackingLevel,
		);

	return includePurchased
		? servers
		: servers.filter((server) => !purchasedServers.includes(server));
};
 ...
 * @return {String[]}
 **/
export const getRootedServers = (ns) =>
	getServersFlattened(ns).filter((server) => ns.hasRootAccess(server));
